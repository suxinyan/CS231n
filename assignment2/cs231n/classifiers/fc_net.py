...
class TwoLayerNet(object):
  ...
  def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
    ...
    ############################################################################
    # W1和W2满足高斯分布且标准差为weight_scale，b1和b2初始化0
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    loss, dscores = softmax_loss(scores, y) # dscores为Loss对得分的偏导
    loss = loss + 0.5 * self.reg * np.sum(self.params['W1'] * self.params['W1']) + 0.5 * self.reg * np.sum(self.params['W2'] * self.params['W2'])
    # dscores为上游传下来的梯度，注意这里还需要使用之前保存下来的缓存，d1_out为经过affine-ReLU层的输出值（用链式法则理解上游传下来的梯度）
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    self.dropout_param = {}
    if self.use_dropout:
      self.dropout_param = {'mode': 'train', 'p': dropout} # train和test两种模式，概率为dropout随机失活
      if seed is not None:
  ...
  def loss(self, X, y=None):
    ...
    mode = 'test' if y is None else 'train' # 输入y为train，不输入y为test，计算loss需要y
    ...
    # layer, etc.                                                              #
    ############################################################################
    ar_cache = {} # 由于是否使用dropout会影响梯度计算，所以中间变量用两个dict保存
    dp_cache = {}
    layer_input = X
    for i in range(1, self.num_layers): # range函数范围前闭后开
        if self.use_batchnorm:
            layer_input, ar_cache[i - 1] = affine_bn_relu_forward(layer_input, self.params['W' + str(i)], self.params['b' + str(i)], self.params['gamma' + str(i)], self.params['beta' + str(i)], self.bn_params[i - 1])
        else:
            layer_input, ar_cache[i - 1] = affine_relu_forward(layer_input, self.params['W' + str(i)], self.params['b' + str(i)])
        if self.use_dropout:
            layer_input, dp_cache[i - 1] = dropout_forward(layer_input, self.dropout_param)
    
    layer_out, ar_cache[self.num_layers] = affine_forward(layer_input, self.params['W' + str(self.num_layers)], self.params['b' + str(self.num_layers)])
    scores = layer_out
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    for i in range(self.num_layers - 1): # 第i个affine-bn-relu层，这些层可以抽象成一层
        layer = self.num_layers - i - 1
        loss += 0.5 * self.reg * np.sum(self.params['W' + str(layer)] * self.params['W' + str(layer)]) # 每个affine-bn-relu层只有一个W
        if self.use_dropout:
